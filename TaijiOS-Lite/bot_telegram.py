...
def main():
    ...
    print("=" * 50)
    print(f"  TaijiOS Telegram Bot 已启动")
        ...
        try:
            ...
            for update in updates.get("result", []):
                ...
                if text == "/start":
                    ...
                    send_message(chat_id, greeting, msg_id)
                    continue

                if text in ("/status", "/状态"):
                    reply = bot.handle_message(user_id, user_name, "status")
                    send_message(chat_id, reply, msg_id)
                    continue

                if text in ("/help", "/帮助"):
                    reply = bot.handle_message(user_id, user_name, "help")
                    send_message(chat_id, reply, msg_id)
                    continue

                if text in ("/kb", "/知识库"):
                    reply = bot.handle_message(user_id, user_name, "kb")
                    send_message(chat_id, reply, msg_id)
                    continue
                ...
                # 处理消息
                reply = bot.handle_message(user_id, user_name, text)
                send_message(chat_id, reply, msg_id)
